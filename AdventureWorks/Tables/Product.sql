CREATE TABLE [Production].[Product] (
    ...
    CONSTRAINT [CK_Product_Class] CHECK (upper([Product].[Class])='H' OR upper([Product].[Class])='M' OR upper([Product].[Class])='L' OR [Product].[Class] IS NULL),
    CONSTRAINT [CK_Product_DaysToManufacture] CHECK ([Product].[DaysToManufacture]>=(0)),
    CONSTRAINT [CK_Product_ListPrice] CHECK ([Product].[ListPrice]>=(0.00)),
    CONSTRAINT [CK_Product_ProductLine] CHECK (upper([Product].[ProductLine])='R' OR upper([Product].[ProductLine])='M' OR upper([Product].[ProductLine])='T' OR upper([Product].[ProductLine])='S' OR [Product].[ProductLine] IS NULL),
    CONSTRAINT [CK_Product_ReorderPoint] CHECK ([Product].[ReorderPoint]>(0)),
    CONSTRAINT [CK_Product_SafetyStockLevel] CHECK ([Product].[SafetyStockLevel]>(0)),
    CONSTRAINT [CK_Product_SellEndDate] CHECK ([Product].[SellEndDate]>=[Product].[SellStartDate] OR [Product].[SellEndDate] IS NULL),
    CONSTRAINT [CK_Product_StandardCost] CHECK ([Product].[StandardCost]>=(0.00)),
    CONSTRAINT [CK_Product_Style] CHECK (upper([Product].[Style])='U' OR upper([Product].[Style])='M' OR upper([Product].[Style])='W' OR [Product].[Style] IS NULL),
    CONSTRAINT [CK_Product_Weight] CHECK ([Product].[Weight]>(0.00)),
